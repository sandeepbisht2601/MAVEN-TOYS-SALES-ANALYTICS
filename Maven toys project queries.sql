...
SELECT
    ...
    CONCAT (
        ROUND(
            SUM(s.units * (p.product_price - p.product_cost)) / SUM(s.units * p.product_price) * 100,
            2
        ),
        '%'
...
SELECT
    ...
    ROUND(SUM(s.units * p.product_price), 2) AS Revenue,
    ROUND(
        SUM(s.units * p.product_price) / COUNT(DISTINCT st.store_id),
        2
    ) AS Rev_per_store
FROM
    sales s
...
SELECT
    ...
    ar.Average_revenue,
    CASE
        WHEN sr.revenue > ar.average_revenue THEN 'Above_average'
        ELSE 'Below_average'
    END AS Performace
FROM
    store_revenue sr
...
WITH
    ...
    Growth AS (
        SELECT
            ...
            Units_sold,
            CASE
                WHEN Units_sold > LAG (Units_sold) OVER (
                    PARTITION BY
                        product_name
                    ORDER BY
                        Ym
